...
namespace BMHRI.WCS.Server.UserControls
{
    ...
    /// </summary>
    public partial class SQLServerHKMsgQueryUC : UserControl
    {
        public SQLServerHKMsgQueryUC()
        {
            InitializeComponent();
        ...
        private void QueryBT_Click(object sender, RoutedEventArgs e)
        {
            ...
            DateTime end_date_time = EndDP.SelectedDate.Value.AddDays(1);

            if (sta_date_time < end_date_time)
                sqlstr += "SDTime >= '" + sta_date_time.ToShortDateString() + "' and SDTime <= '" + end_date_time.ToShortDateString() + "'";
            else if (sta_date_time == end_date_time)
                sqlstr += "SDTime >= '" + sta_date_time.ToShortDateString() + "'";
            if (!string.IsNullOrEmpty(str))
                sqlstr += " AND ReqMsg LIKE('%" + str + "%') ";
            sqlstr += " ORDER BY SDTime DESC";
        ...
        private void WMSTaskLV_MouseDoubleClick(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            if (lv.DataContext is DataRowView dataView)
            {
                ...
                HaiKangMessageDetailWindows wMSMessageDetailWindows = new(dataView);
