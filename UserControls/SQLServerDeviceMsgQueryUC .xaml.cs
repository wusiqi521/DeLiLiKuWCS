...
namespace BMHRI.WCS.Server.UserControls
{
    ...
    /// </summary>
    public partial class SQLServerDeviceMsgQueryUC : UserControl
    {
        public SQLServerDeviceMsgQueryUC()
        {
            InitializeComponent();   
        }
        ...
        private void UserControl_Loaded(object sender, RoutedEventArgs e)
        {
            if (IsDeviceMsgQuery)
            {
                ...
                {
                    List<PLCInfor> PLCInforList = new List<PLCInfor>();

                    PLCInforList.Add(new PLCInfor("请选择设备", "0"));
                    foreach ( SSJDevice sSJDevice in PLCDeviceManager.Instance.SSJDeviceList)
                    {
                        PLCInforList.Add(new PLCInfor(sSJDevice.PLCDecription, sSJDevice.PLCID));
                    }
                    foreach (DDJDevice dDJDevice in PLCDeviceManager.Instance.DDJDeviceList)
                    {
                        PLCInforList.Add(new PLCInfor(dDJDevice.PLCDecription, dDJDevice.PLCID));
                    }
    ...
    class PLCInfor
    {
        public PLCInfor(string v1, string v2)
        {
            Name = v1;
            PLCID = v2;
        }
